...
def merkle_assignment():
    ...
    random_leaf_index = random.randint(0, len(leaves)-1) #TODO generate a random index from primes to claim (0 is already claimed)
...
def build_merkle(leaves):
    ...
    while len(current) > 1:
        ...
        for even_i in range(0, len(current), 2):
            left = current[even_i]
            right = current[even_i+1]
            next_height.append(hash_pair(left, right))

        tree.append(next_height)
        current = next_height
    return tree
...
def prove_merkle(merkle_tree, random_indx):
    ...
    for level in merkle_tree[:-1]:
        ...
        sibling_index = random_indx ^ 1 # XOR function
