...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		require(ERC20(sellToken).transferFrom(msg.sender, address(this), sellAmount), "WARNING: Failed Transfer");
		qtyA = ERC20(tokenA).balanceOf(address(this));
		qtyB = ERC20(tokenB).balanceOf(address(this));

		uint256 fee = (sellAmount*feebps)/10000;
		uint256 total_amount = sellAmount-fee;
		uint256 actualSell;
		uint256 actualBuy;
		uint256 new_sell;
		uint256 new_buy;
		...
		if (sellToken == tokenA){
			actualSell = qtyA - sellAmount;
			actualBuy = qtyB;
		} else{
			actualSell = qtyB - sellAmount;
			actualBuy = qtyA;
		}

		new_sell = actualSell + total_amount;
		new_buy = invariant / new_sell;
		require(actualBuy > new_buy, "Insufficient liquidity");
		swapAmt = actualBuy - new_buy;

		require(ERC20(buyToken).transfer(msg.sender, swapAmt), "WARNING: Failed Transfer");
	...
	*/
	function provideLiquidity( uint256 amtA, uint256 amtB ) public {
		...
		require(ERC20(tokenA).transferFrom(msg.sender, address(this), amtA), "WARNING: Unable to Transfer tokenA");
		require(ERC20(tokenB).transferFrom(msg.sender, address(this), amtB), "WARNING: Unable to Transfer tokenB");
